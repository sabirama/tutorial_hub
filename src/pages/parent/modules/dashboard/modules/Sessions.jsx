    ...
    <div className="sessions-page">
      ...
      <div className="calendar-controls">
        <div className="view-controls">
          <button className={`view-btn ${view === 'list' ? 'active' : ''}`} onClick={() => setView('list')}>List View</button>
      ...
      {view === 'list' && (
        <div className="sessions-grid">
          {filteredSessions.length === 0 ? (
            <div className="no-sessions"><p>No sessions found matching your criteria.</p></div>
          ) : (
              ...
              <div key={session.id} className="session-card">
                ...
                <div className="session-actions">
                  {session.status === 'upcoming' && (
                    <>
                      <button className="btn-secondary" onClick={() => handleReschedule(session.id)}>Reschedule</button>
                      <button className="btn-cancel" onClick={() => handleCancel(session.id)}>Cancel</button>
                    </>
                  )}
                </div>
              </div>
                ...
                <div className="week-sessions">
                  {day.sessions.map(session => (
                    <div key={session.id} className="week-session-item" style={{ borderLeftColor: getStatusColor(session.status) }}
                      ...
                      <div className="week-session-tutor">{session.tutor_name}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="slot-sessions">
                  {slot.sessions.map(session => (
                    <div key={session.id} className="session-card-mini" style={{ borderLeftColor: getStatusColor(session.status) }}>
                      ...
                      <div className="session-tutor">{session.tutor_name}</div>
          ...
          <div className="session-modal">
            ...
            <div className="session-details">
              ...
              <div className="participants-section">
                <h4>Participants</h4>
                <div className="participant-detail"><span>Tutor:</span> <span>{selectedSession.tutor_name}</span></div>
                <div className="participant-detail"><span>Student:</span> <span>{selectedSession.child_name}</span></div>
              </div>
              {selectedSession.notes && (
                <div className="notes-section">
                  <h4>Notes</h4>
                  <div className="notes-content"><p>{selectedSession.notes}</p></div>
                </div>
              )}
              {selectedSession.status === 'upcoming' && (
                <div className="session-actions-modal">
                  <button className="btn-secondary" onClick={() => handleReschedule(selectedSession.id)}>Reschedule</button>
                  <button className="btn-cancel" onClick={() => handleCancel(selectedSession.id)}>Cancel Session</button>
                </div>
              )}
            </div>
          </div>
